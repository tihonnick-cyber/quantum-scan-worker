...
// Despite the legacy TOP20_* variable namespace, the scanner can now watch a larger
...
let top20LastStats          = {};
let top20LastTriggerDiagnostics = [];
let top20LastHistoryCleanupAt = 0;
...
app.get("/top20", (_req, res) => {
  ...
  res.json({
    ...
    scanner: `TOP ${TOP20_GAINERS_LIMIT} SCALP`,
    ...
    universeSize: TOP20_GAINERS_LIMIT,
    leadersCount: top20LastLeaders.length,
    triggerDiagnostics: top20LastTriggerDiagnostics,
...
// Alias for the widened Top-50 scanner. /top20 remains for Base44 backward compatibility.
...
app.get("/top50_history", async (req, res) => {
  try {
    await ensureTop20ScanHistoryTable();
    const requested = Number(req.query?.limit || 120);
    const limit = Math.max(1, Math.min(500, Number.isFinite(requested) ? requested : 120));
    const result = await pool.query(
      `SELECT id, scanned_at, universe_size, leaders
       FROM top20_scan_history
       ORDER BY scanned_at DESC
       LIMIT $1`,
      [limit]
    );
    res.json({ ok: true, count: result.rows.length, scans: result.rows });
  } catch (e) {
...
// Legacy alias.
app.get("/top20_history", async (req, res) => {
  try {
    await ensureTop20ScanHistoryTable();
    const requested = Number(req.query?.limit || 120);
    const limit = Math.max(1, Math.min(500, Number.isFinite(requested) ? requested : 120));
...
app.get("/top20_test", async (_req, res) => {
  try {
    const result = await pushToTelegram(
      `🔥 <b>TOP ${TOP20_GAINERS_LIMIT} TECHNICAL — TEST 5/5</b>\n` +
...
// ===== TOP-GAINER SCAN HISTORY / TRIGGER DIAGNOSTICS =====
let top20ScanHistoryInitPromise = null;

async function ensureTop20ScanHistoryTable() {
  if (top20ScanHistoryInitPromise) return top20ScanHistoryInitPromise;
  top20ScanHistoryInitPromise = (async () => {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS top20_scan_history (
        id BIGSERIAL PRIMARY KEY,
    ...
    `);
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_top20_scan_history_scanned_at
      ON top20_scan_history (scanned_at DESC)
    `);
  })().catch(err => {
    top20ScanHistoryInitPromise = null;
    throw err;
  });
  return top20ScanHistoryInitPromise;
}

async function recordTop20ScanHistory(rows) {
  if (!TOP20_SCAN_HISTORY_ENABLED || !Array.isArray(rows)) return;
  try {
    await ensureTop20ScanHistoryTable();
    await pool.query(
      `INSERT INTO top20_scan_history (universe_size, leaders) VALUES ($1, $2::jsonb)`,
      [TOP20_GAINERS_LIMIT, JSON.stringify(rows)]
    );

    // Cleanup at most once per hour so retaining history does not add work to every scan.
    if (Date.now() - top20LastHistoryCleanupAt > 60 * 60 * 1000) {
      top20LastHistoryCleanupAt = Date.now();
      await pool.query(
        `DELETE FROM top20_scan_history
         WHERE scanned_at < now() - ($1 || ' hours')::interval`,
        [String(TOP20_SCAN_HISTORY_RETENTION_HOURS)]
      );
    }
  } catch (e) {
...
function formatTop20Telegram(result, timing = {}) {
  ...
  const tv = `https://www.tradingview.com/symbols/${encodeURIComponent(ticker)}/`;
  const title = score >= 5
    ? `🔥 <b>TOP ${TOP20_GAINERS_LIMIT} SCALP — 5/5</b>`
    : `🟠 <b>TOP ${TOP20_GAINERS_LIMIT} SCALP — 4/5</b>`;
...
function completedMinuteKey() {
  // IMPORTANT: shift the key by the same completion grace used by the trigger.
  // Without this, a request at xx:xx:00 can cache bars before the just-closed
  // candle has arrived, causing the scanner to miss that 5x candle for a full minute.
  return Math.floor((Date.now() - TOP20_TRIGGER_COMPLETION_GRACE_MS) / 60_000) - 1;
}
...
function evaluateTop20Trigger(bars) {
  const empty = {
    ...
    currentCandleTimestampMs: null,
    previousCandleTimestampMs: null,
    barsOneMinuteApart: false,
  ...
  const barsOneMinuteApart = Number(current.t) - Number(previous.t) === 60_000;
  ...
  return {
    ...
    currentCandleTimestampMs: Number(current.t),
    previousCandleTimestampMs: Number(previous.t),
    barsOneMinuteApart,
...
async function scanTop20Technicals() {
  ...
  try {
    ...
    const evaluated = await runWithConcurrency(
      ...
      async leader => {
        try {
          ...
          // Trigger gate: compare the LAST TWO fully completed 1-minute candles.
          const trigger = evaluateTop20Trigger(bars);

          const diagnostic = {
            rank: leader.rank,
            ticker: leader.ticker,
            price: Number(leader.price.toFixed(4)),
            pct: Number(leader.pct.toFixed(2)),
            dayVolume: Math.round(leader.volume),
            previous1mVolume: Number(trigger.previousCandleVolume || 0),
            current1mVolume: Number(trigger.currentCandleVolume || 0),
            volumeMultiplier: Number(trigger.multiplier || 0),
            previousBarAt: trigger.previousCandleTimestampMs
              ? new Date(trigger.previousCandleTimestampMs).toISOString() : null,
            currentBarAt: trigger.currentCandleTimestampMs
              ? new Date(trigger.currentCandleTimestampMs).toISOString() : null,
            barsOneMinuteApart: Boolean(trigger.barsOneMinuteApart),
            uptrend: Boolean(trigger.uptrendConfirmed),
            priceAboveSma10: Boolean(trigger.priceAboveSma10),
            sma10Above100: Boolean(trigger.sma10Above100),
            sma10Rising: Boolean(trigger.sma10Rising),
            sma100Rising: Boolean(trigger.sma100Rising),
            green: Boolean(trigger.triggerCandleGreen),
            higherClose: Boolean(trigger.higherClose),
            triggerPassed: Boolean(trigger.passed),
            triggerReason: String(trigger.reason || "UNAVAILABLE"),
          };
          triggerDiagnostics.push(diagnostic);

          // Console-log meaningful volume acceleration so Railway makes the math visible.
          if (trigger.multiplier >= TOP20_TRIGGER_DEBUG_MULTIPLIER || trigger.passed) {
            console.log(
              `[TOP${TOP20_GAINERS_LIMIT}][TRIGGER] #${leader.rank} ${leader.ticker} ` +
              `${trigger.currentCandleVolume}/${trigger.previousCandleVolume}=` +
              `${Number(trigger.multiplier || 0).toFixed(2)}x ` +
              `uptrend=${trigger.uptrendConfirmed} green=${trigger.triggerCandleGreen} ` +
              `higherClose=${trigger.higherClose} passed=${trigger.passed} reason=${trigger.reason}`
            );
          }
    ...
    // Keep diagnostics sorted by current gainer rank and persist a compact snapshot.
    triggerDiagnostics.sort((a, b) => a.rank - b.rank);
    top20LastTriggerDiagnostics = triggerDiagnostics;
    await recordTop20ScanHistory(triggerDiagnostics);
    ...
    top20LastStats = {
      ...
      shadowThisRun,
      triggerPasses,
      universeLimit: TOP20_GAINERS_LIMIT,
      circuitOpen: top20CircuitOpen,
    };
